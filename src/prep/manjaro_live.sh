...
# Configurations
configs() {
    setfont lat9-16 # /usr/share/kbd/consolefonts/
...
fixdb() {
  # Ignore signature checking in pacman.conf (insecure) - use fix below (slow)
    ...
    pacman -U /var/cache/pacman/pkg/manjaro-keyring*.pkg.tar.xz
    ...
    pacman -Syvv --noconfirm archlinux-keyring manjaro-keyring
}

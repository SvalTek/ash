...
def ash_update():
    try:
        ...
        subprocess.check_output(f"curl --fail -H 'pragma:no-cache' -H 'cache-control:no-cache,no-store' -s -o {tmp_ash}/ashpk_core.py -O \
                                'https://raw.githubusercontent.com/svaltek/ash/latestsrc/ashpk_core.py'", shell=True) # GitHub still caches
        subprocess.check_output(f"curl --fail -H 'pragma:no-cache' -H 'cache-control:no-cache,no-store' -s -o {tmp_ash}/ashpk.py -O \
                                'https://raw.githubusercontent.com/svaltek/ash/latestsrc/distros/{d}/ashpk.py'", shell=True)
...
def install_profile(snapshot, profile):
    if not os.path.exists(f"/.snapshots/rootfs/snapshot-{snapshot}"):
        ...
        subprocess.check_output(f"curl --fail -o {tmp_prof}/packages.txt -LO https://raw.githubusercontent.com/svaltek/ash/latestsrc/profiles/{profile}/packages{get_distro_suffix()}.txt", shell=True)
        prepare(snapshot)
...
def install_triage(snapshot, live, profile, pkg, not_live):
  ...
  # Perform the live install only if install above was successful
    if live and not excode:
        ### BUG: pakages from AUR fail to install correctly here, installation is successful but package is not found.
        if profile:
